...
begin
    ...
    for i in 1..v_klij.count() loop
        dbms_output.put_line(v_klij(i).ime_var||' '||v_klij(i).prez_var||' '||v_klij(i).tit_var||' '||v_klij(i).stat_var||' '||v_klij(i).dat_zap_var||' '||v_klij(i).id_od_var);
    end loop;

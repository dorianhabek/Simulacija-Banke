create or replace PROCEDURE TRANSAKCIJA(fr_acc number, to_acc number, doznaka varchar2, iznos number) AS 
    ...
    cursor cur_trans is select serijski_broj from transakcije;
    
    fr_rac_row cur_fr_acc%rowtype;
    to_rac_row cur_to_acc%rowtype;
    trans_row cur_trans%rowtype;
...
begin
    ...
    open cur_trans;
    fetch cur_trans into trans_row;
    not_found := cur_trans%notfound;
    close cur_trans;
    if not_found then
        next_id := 1; --u slučaju novonastale tablice
    else
